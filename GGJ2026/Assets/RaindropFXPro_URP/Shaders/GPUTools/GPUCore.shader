Shader "Custom/RaindropFX/GPUCore" {
    Properties {
		_MainTex("Base", 2D) = "" {}
		_NoiseTex("Noise", 2D) = "" {}
	...
	// color level
	float GetPixelLevel(float inPixel, float shrinkSpeed) {
	...
	half4 frag(v2f i) : COLOR {
		...
		float fade = clamp(randomNum, lerp(0.92, 0.98, _Lifetime.x), lerp(0.95, 0.99, _Lifetime.x));
		float fade_sta = clamp(randomNum, lerp(0.9, 0.98, _Lifetime.y), lerp(0.92, 0.99, _Lifetime.y));

		float4 pCol = tex2D(_MainTex, i.uv);
		float4 mainColor = float4(0.0, 
			//max(pCol.r, GetPixelLevel(pCol.g * 0.9, i.uv)), 
			//max(pCol.r, GetPixelLevel(BoxFilter(i.uv) * fade_sta, shrink)), 
			//max(pCol.r, GetPixelLevel(tex2D(_MainTex, i.uv).g * fade_sta, shrink)),
			max(pCol.r, tex2D(_MainTex, i.uv).g * fade),
			max(_fadeout * randomDropFast(
					i.uv * _MainTex_TexelSize.zw, _Time.x * 5.0, 
					randomNum * 24.0 * _Force_RaindropSize.z, _staDens, false
				), 
				//GetPixelLevel(pCol.b * fade_sta, shrink)
				pCol.b * fade_sta
			), 1.0
		);

		//float perlin = (perlin_noise(i.uv * 8.0) + 1.0) / 2.0;
		//float2 sp = i.uv - (tex2D(_TurbulenceLUT, i.uv).xy * perlin * 15.0 + _Force_RaindropSize.xy) * _MainTex_TexelSize.xy;
		...
		float2 dir = normalize(_Force_RaindropSize.xy);
		float acc = 0.1 + DistanceField(i.uv, -dir);
		float2 turb = _TurbScale < epsilon ? float2(0, 0) : (RTDir(dir) * Turbulence(float2(i.uv.x, acc) * 256.0) * _TurbScale * 0.001);
		float2 sp = i.uv - turb - _Force_RaindropSize.xy * acc * 0.0032 - SamplingForceLUT(i.uv) * 0.005;// *_MainTex_TexelSize.zw;
		if (sp.x >= 0 && sp.x <= 1 && sp.y >= 0 && sp.y <= 1)
			mainColor.r = max(
				//GetPixelLevel(tex2D(_MainTex, sp).r * lerp(fade_sta, 1.0, _Lifetime.y), shrink * 8.0),
				GetPixelLevel(tex2D(_MainTex, sp).r, shrink * 8.0),
				(_SinTime.w * _fadeout) == 0 ? 0.0 :
				randomDropFast(
					i.uv * _MainTex_TexelSize.zw, 
					randomNum * 100.0, 
					clamp(randomNum, 0.8, 1.5) * 6.0 * _Force_RaindropSize.w,
					_dynDens, true
				)
			);
