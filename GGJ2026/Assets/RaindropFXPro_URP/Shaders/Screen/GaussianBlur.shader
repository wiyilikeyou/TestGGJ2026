		...
		SubShader{
			ZTest Always ZWrite Off Cull Off Fog { Mode Off }
				...
				float4 fragDownsample(vs2psDown IN) : COLOR {
					float4 c = 0;
					for (uint i = 0; i < 4; i++) c += tex2D(_MainTex, IN.uv[i]) * 0.25;
					return c;
				}
				...
				vs2psBlur vertBlurH(vsin IN) {
					vs2psBlur OUT;
					OUT.vertex = UnityObjectToClipPos(IN.vertex);
					for (uint i = 0; i < 8; i++) OUT.uv[i] = IN.uv + float2(OFFSETS[i], 0) * _MainTex_TexelSize.xy;
					return OUT;
				}
				vs2psBlur vertBlurV(vsin IN) {
					vs2psBlur OUT;
					OUT.vertex = UnityObjectToClipPos(IN.vertex);
					for (uint i = 0; i < 8; i++) OUT.uv[i] = IN.uv + float2(0, OFFSETS[i]) * _MainTex_TexelSize.xy;
					return OUT;
				}
				float4 fragBlur(vs2psBlur IN) : COLOR {
					float4 c = 0;
					for (uint i = 0; i < 8; i++) c += tex2D(_MainTex, IN.uv[i]) * WEIGHTS[i];
					return c;
				}
				ENDCG
