...
namespace RaindropFX {
    public class MaterialLinkerBase : MonoBehaviour {
        ...
        public RenderTexture tempD = null;
        ...
        public void Solve() {
            if (_raindropTex_alpha != null) {
                ...
                solver.CalcRainTex();
                solver.GenerateTextures(
                    ref calcRainTex, ref fogMask, ref wipeMask, 
                    ref wipeDelta, ref tempD
                );
            } else {
                if (solver.raindropTex_alpha == null)
        ...
        private void SendPropertiesToSolver() {
            solver.UpdateProps(
                fadeout_fadein_switch, fastMode, fadeSpeed, forceRainTextureSize, 
                calcRainTextureSize, calcTimeStep, refreshRate, generateTrail, 
                maxStaticRaindropNumber, maxDynamicRaindropNumber, raindropSizeRange, 
                useWind, windTurbulence, windTurbScale, wind, gravity,
                friction, distortion, useFog, fogIntensity, fogIteration, 
                fusion, _inBlack, _inWhite, _outWhite, _outBlack, 
                dropletBlur, _focalize, blurIteration, tintColor, tintWeight, 
                fogTint, radialWind, _rainMask_grayscale, pixelization, 
                pixResolution, downSampling, foggingSpeed
            );
        ...
        private void Constraints() {
            ...
            if (fusion < 0) fusion = 0;
            else if (fusion > 15) fusion = 15;
