...
namespace RaindropFX {
    public class RaindropGenerator {
        ...
        public Material kawase_blur_material;
        ...
        public List<Raindrop_URP> staticRaindrops;
        public List<Raindrop_URP> dynamicRaindrops;
        public CachePool_URP cachePool;
        ...
        public void Init(Texture2D raindropTex_alpha, Vector2Int calcRainTextureSize) {
            ...
            cachePool = Camera.main.GetComponent<CachePool_URP>();
            if (!cachePool) cachePool = Camera.main.gameObject.AddComponent<CachePool_URP>();
            else cachePool.Init();
            ...
            generateBaseMap();
            staticRaindrops = new List<Raindrop_URP>();
            dynamicRaindrops = new List<Raindrop_URP>();
            PreparRainTexFlags();

            if (blend_material == null) {
                blend_material = new Material(Shader.Find("Hidden/Custom/ScreenBlend"));
            }
            if (blur_material == null) {
                blur_material = new Material(Shader.Find("Hidden/Custom/GaussianBlur"));
            }
            if (kawase_blur_material == null) {
                kawase_blur_material = new Material(Shader.Find("Hidden/Custom/KawaseBlur"));
            }
        ...
        public void UpdateProps(
            ...
            bool useWind, float windTurbulence, float windTurbScale, Vector2 wind, Vector2 gravity, 
            float friction, float distortion, bool useFog, float fogIntensity, int fogIteration, 
            int edgeSoftness, float _inBlack, float _inWhite, float _outWhite, float _outBlack, 
            bool dropletBlur, float _focalize, int blurIteration, Color tintColor, int tintWeight, 
            Color fogColor, bool radialWind, Texture cullMask, bool pixelization, float pixResolution, float downSampling, float foggingSpeed
        ) {
            if (forceRainTextureSize) {
        ...
        //----------------------------------------------------------------
        public void PaintToCanvas(Raindrop_URP raindrop) {
        ...
        // Don't forget to call ApplyTexPainting(); after that
        //---------------------------------------------------------------------
        public void EraseFromCanvas(Raindrop_URP raindrop) {
            int widthOfRain = (int)raindrop.realSize.x;
            int heightOfRain = (int)raindrop.realSize.y;
            int left_up_X = (int)(raindrop.position.x - widthOfRain / 2);
            int left_up_Y = (int)(raindrop.position.y - heightOfRain / 2);
        ...
        //---------------------------------------------------------------------
        public bool Shrink(Raindrop_URP raindrop, float shrinkAmount) {
        ...
        //---------------------------------------------------------------------
        public bool Fade(Raindrop_URP raindrop, float fadeAmount) {
            int widthOfRain = (int)raindrop.realSize.x;
            int heightOfRain = (int)raindrop.realSize.y;
            int left_up_X = (int)(raindrop.position.x - widthOfRain / 2);
            int left_up_Y = (int)(raindrop.position.y - heightOfRain / 2);
        ...
        public void generateBaseMap() {
            calcRainTex = new Texture2D(calcTexSize.x, calcTexSize.y, TextureFormat.RGB24, false);
        ...
        public void GenerateTextures(
            ref RenderTexture resultTex, ref RenderTexture fogMask, 
            ref RenderTexture wipeMask, ref RenderTexture wipeDelta,
            ref RenderTexture tempD
        ) {
            ...
            // apply wipe effect
            if (wipeMask == null || wipeMask.width != calcTexSize.x)
                wipeMask = new RenderTexture(calcTexSize.x, calcTexSize.y, 0);
            if (tempWip == null || tempWip.width != calcTexSize.x)
                tempWip = new RenderTexture(calcTexSize.x, calcTexSize.y, 0);
            if (tempD == null || tempD.width != calcTexSize.x)
                tempD = new RenderTexture(calcTexSize.x, calcTexSize.y, 0);
            if (wipeEffect && wipeDelta != null) {
                if (cnt) {
                    RaindropFX_Tools.GaussianBlur(wipeMask, tempD, 1, blur_material);
                } else {
                    SetTimeMat(wipeDelta);
                ...
                RaindropFX_Tools.RenderTextureToTexture2D(tempWip, calcRainTex);
            }

            // apply blur effect to calcRainTex
            RaindropFX_Tools.GaussianBlur(
                calcRainTex, temp, edgeSoftness, blur_material
            );
            ...
            // disable pixelize on surface
            pixel_material.SetFloat("_PixelSize", 0);
        ...
        public void SetTimeMat(RenderTexture wipeDelta) {
        ...
        public void SetScreenBlendMat(
            ...
            blend_material.SetVector(
                "_RainTex_TexelSize", 
                new Vector4(1.0f / calcTexSize.x, 1.0f / calcTexSize.y, 0, 0)
            );

            blend_material.SetTexture("_RFX_HeightMap", tempHeightMap);
            blend_material.SetTexture("_RFX_WetTex", tempWetMap);
            blend_material.SetTexture("_RFX_WipeMap", wipeMap);
            blend_material.SetTexture("_RFX_CullMask", cullMask_grayscale);
        }
        public void SetDropblurMat(ref RenderTexture tempMask, Texture staticMask) {
        ...
        //--------------------------
        public void Kill(Raindrop_URP p, bool isStatic) {
            if (isStatic) {
                EraseFromCanvas(p);
                cachePool.Recycle(p);
        ...
        public void GenTrailDrop(Vector2 centerPos, float size) {
            ...
            Raindrop_URP newRaindrop = cachePool.GetRaindrop();
            newRaindrop.Reuse(
                true, centerPos, size, 
