...
public class AudioManager : Singleton<AudioManager>
{
    ...
    public int PlayOneShot(AudioClip clip, float starttime = 0f,float volume = 1f, float pan = 0f, float pitch = 1.0f)
    {
        ...
        for (int i = 0; i < m_channels.Length; i++)
        {
            ...
            if (!m_channels[i].channel.isPlaying)
            {
                ...
                m_channels[i].channel.time = starttime;
    ...
    public int PlayLoop(AudioClip clip, float volume = 1, float pan = 0, float pitch = 1.0f)
    {
        for (int i = 0; i < m_channels.Length; i++)
        {
            //如果正在播放同一个片段，而且刚刚才开始，则直接退出函数
